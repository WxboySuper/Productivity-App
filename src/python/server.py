...
# Early error logging before Flask setup
def log_startup_error(error):
    try:
        app_data = os.getenv('APPDATA') or os.path.expanduser('~/Library/Application Support')
        log_dir = os.path.join(app_data, 'ProductivityApp', 'logs')
        os.makedirs(log_dir, exist_ok=True)
        
        with open(os.path.join(log_dir, 'startup_error.log'), 'a') as f:
            f.write(f'[{time.strftime("%Y-%m-%d %H:%M:%S")}] Startup Error:\n')
            f.write(f'{str(error)}\n')
            f.write(traceback.format_exc())
            f.write('\n')
    except Exception as e:
        print(f"Failed to log startup error: {e}", file=sys.stderr)
