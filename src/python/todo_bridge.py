...
def handle_command(cmd, payload):
    ...
    """
    with log_context(log, "handle_command", command=cmd):
        ...
        try:
            if cmd == "initialize":
                log.info("Initializing bridge [RequestID: %s]", cmd_request_id)
...
if __name__ == "__main__":  # pragma: no cover
    try:
        ...
        if len(sys.argv) < 3:
            log.error(f"Received {len(sys.argv)-1} arguments, expected 2")
            log.debug(f"Arguments received: {sys.argv[1:]}")
